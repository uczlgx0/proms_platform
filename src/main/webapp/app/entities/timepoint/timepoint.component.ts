...
@Component({
    selector: 'jhi-timepoint',
    templateUrl: './timepoint.component.html'
})
export class TimepointComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/timepoint'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Timepoint) {
        return item.id;
    }
    registerChangeInTimepoints() {
        this.eventSubscriber = this.eventManager.subscribe('timepointListModification', (response) => this.loadAll());
    }
