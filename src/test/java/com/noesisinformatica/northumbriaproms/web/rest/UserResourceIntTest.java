...
import com.noesisinformatica.northumbriaproms.service.dto.UserDTO;
import com.noesisinformatica.northumbriaproms.service.mapper.UserMapper;
import com.noesisinformatica.northumbriaproms.web.rest.errors.ExceptionTranslator;
import com.noesisinformatica.northumbriaproms.web.rest.vm.ManagedUserVM;
...
public class UserResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllUsersWithAuthority() throws Exception {
        restUserMockMvc.perform(get("/api/users/authority/{authority}/?sort=id,desc", AuthoritiesConstants.ADMIN)
            .accept(TestUtil.APPLICATION_JSON_UTF8)
            .contentType(TestUtil.APPLICATION_JSON_UTF8))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$").isArray())
            .andExpect(jsonPath("$.[*].login").value(containsInAnyOrder("admin", "system")));
    }
